...
- `workflow/server.js` is a dependency-free `http` server. It serves `xray-md-evidence.html` at `/`, and exposes a JSON API for the Xray automation:
  ...
  - `POST /workflow/start`, `GET /workflow/:id`, `POST /workflow/:id/cancel` — async run lifecycle; runs are tracked in-memory (`runs` Map) with logs. Runs execute one at a time through a server-side FIFO queue (`runQueue`/`processQueue`/`executeRun`) against a single shared Playwright browser/context/page (`workflow/xray-workflow.js`'s `getSharedBrowserContext`/`getSharedPage`, reused across every run rather than relaunched per run) — this exists because concurrent logins against Jira/Xray cause it to invalidate the shared session. Cancellation is cooperative via an `AbortController` per run (`throwIfWorkflowCancelled` checkpoints inside `runXrayWorkflow`); it never closes the shared browser, only removes a not-yet-started run from the queue or aborts the in-flight one so the queue can move on.
